...
export function NextProject({ currentSlug, nextProjectImage }: NextProjectProps) {
  ...
  // Low-end: Static version without animations
  if (isLowEnd) {
    return (
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12">
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-8">
                  <div className="flex flex-col gap-2 md:gap-4">
                    <h3 className="font-['Barlow_Semi_Condensed',sans-serif] font-medium text-[28px] md:text-[48px] lg:text-[64px] tracking-[0.05em] uppercase text-white leading-none">
                      {nextProject.title}
                    </h3>
                  ...
                  {/* CTA Button */}
                  <button className="flex items-center gap-3 px-6 py-3 md:px-8 md:py-4 bg-[#50C1BA] text-black font-['Barlow',sans-serif] font-medium text-[14px] md:text-[16px] tracking-[0.15em] uppercase rounded-full self-start md:self-auto">
  ...
  // High/Medium: Full animated version
  return (
    <section className="w-full border-t border-gray-800">
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-16 md:py-24">
        <motion.div
        ...
        >
          ...
          <div className="flex items-center justify-between">
            ...
            <motion.div
              whileHover={config.enableHoverEffects ? { x: 8 } : undefined}
          ...
          <motion.div
            onClick={handleClick}
            className="group cursor-pointer relative overflow-hidden rounded-[24px] md:rounded-[40px]"
            whileHover={config.enableHoverEffects ? { scale: 1.01 } : undefined}
          ...
          >
            ...
            <div className="relative w-full aspect-[16/9] md:aspect-[21/9] overflow-hidden">
              <OptimizedImage
                src={nextProjectImage}
                alt={nextProject.title}
                className={`w-full h-full object-cover ${config.enableHoverEffects ? 'transition-transform duration-700 group-hover:scale-105' : ''}`}
                containerClassName="w-full h-full"
              />
              {/* Gradient Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12">
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: config.animationDuration * 0.7, delay: 0.2 }}
                className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-8"
              >
                <div className="flex flex-col gap-2 md:gap-4">
                  <h3 className={`font-['Barlow_Semi_Condensed',sans-serif] font-medium text-[28px] md:text-[48px] lg:text-[64px] tracking-[0.05em] uppercase text-white leading-none ${config.enableHoverEffects ? 'group-hover:text-[#50C1BA] transition-colors duration-300' : ''}`}>
                    {nextProject.title}
                  </h3>
                  <p className="font-['Lato',sans-serif] font-light text-[14px] md:text-[18px] tracking-[0.02em] text-gray-300 max-w-xl hidden md:block">
                    {nextProject.description}
                ...
                {/* CTA Button */}
                <motion.button
                  whileHover={config.enableHoverEffects ? { scale: 1.05 } : undefined}
                  whileTap={config.enableHoverEffects ? { scale: 0.98 } : undefined}
                  className={`flex items-center gap-3 px-6 py-3 md:px-8 md:py-4 bg-[#50C1BA] text-black font-['Barlow',sans-serif] font-medium text-[14px] md:text-[16px] tracking-[0.15em] uppercase rounded-full ${config.enableHoverEffects ? 'transition-all duration-300 hover:bg-white' : ''} self-start md:self-auto`}
                >
                  <span>View Project</span>
                  <ArrowRight size={18} />
                </motion.button>
